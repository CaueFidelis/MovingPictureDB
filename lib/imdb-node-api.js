...
// Per Year
async function searchMoviesPerYear({ page, year }) {
  let allMovies = [];
  let promises = [];

  const requestUrl = `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=pt-BR&page=${page}&sort_by=popularity.desc&year=${year}`;
  const options = {
    headers: {
      accept: "application/json",
      Authorization:
